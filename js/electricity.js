...
function getTopUps() {
    topups = [];
    $("#topups").find("*").remove();
    showProgress("Memuat daftar transaksi top up listrik");
    $.ajax({
        type: 'GET',
        url: PHP_PATH+'get-electricity-topups.php',
        dataType: 'text',
        cache: false,
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $("#topups").append("" +
                ...
                "<td>" + buyerName + "</td>" +
                "<td>" + formatMoney(parseInt(topup["nominal"])) + " kWh</td>" +
    ...
    $(".view-order").unbind().on("click", function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $("#view-topup-buyer-token").val(topup["token"]);
                $("#view-topup-nominal").val(parseInt(topup["nominal"])+" kWh");
                ...
                $("#verify-paid").unbind().on("click", function() {
                    ...
                    $("#confirm-ok").html("Ya").unbind().on("click", function() {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: PHP_PATH+'verify-pending-electricity-topup.php',
                            data: fd2,
                            processData: false,
                            contentType: false,
                            cache: false,
